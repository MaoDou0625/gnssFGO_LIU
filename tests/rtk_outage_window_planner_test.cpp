...
void TestSegmentedBatchRunnerPassesBoundaryAttitudeReferenceWithoutStage2Timeline() {
  ...
  ExpectTrue(calls[1].reference->boundary_references.size() == 1U,
             "post child should receive one recovery-derived soft boundary");
  ExpectTrue(calls[1].reference->boundary_references.front().boundary_role == "POST_START",
             "post soft boundary should target POST_START");
  ExpectTrue(calls[1].reference->initial_value_boundary_references.empty(),
             "post child should not need a separate initial-only boundary");

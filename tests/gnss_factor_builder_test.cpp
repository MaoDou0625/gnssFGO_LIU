...
void TestBuilderCanDisableAllGnssFactorsForStage3ReferencePass() {
  auto config = offline_lc_minimal::DefaultConfig();
  config.vertical_constraint_mode = offline_lc_minimal::VerticalConstraintMode::kDirectZ;
  config.early_gnss_relaxation_duration_s = 0.0;
  ...
  request.collect_consistency_records = true;
  request.dynamic_start_time_s = 1.0;
  request.disable_horizontal_factors = true;
  request.disable_vertical_factors = true;
  request.should_use_sample = [](const auto &) { return true; };
  request.is_within_imu_coverage = [](double) { return true; };
  request.corrected_time_s = [](const auto &sample) { return sample.time_s; };
  ...
  ExpectTrue(graph.empty(), "Stage3 reference pass should add no raw GNSS factors");
  ExpectNear(static_cast<double>(summary.gnss_factor_count), 0.0, 0.0, "disabled GNSS factors should not count as used");
  ExpectNear(
    static_cast<double>(summary.gnss_synced_factor_count),
    0.0,
    0.0,
    "disabled synchronized GNSS should not count as synced factors");
  ExpectTrue(factor_records.size() == 1U, "disabled sample should still keep diagnostics");
  ExpectTrue(consistency_records.size() == 1U, "disabled sample should still keep consistency diagnostics");
  ExpectTrue(!factor_records.front().factor_used, "disabled sample should not mark factor_used");
  ExpectTrue(
    !factor_records.front().vertical_direct_position_factor_used,
    "disabled sample should not report a direct vertical factor");
  ExpectTrue(!consistency_records.front().factor_used, "disabled consistency should not mark factor_used");
  ExpectTrue(
    !consistency_records.front().vertical_direct_position_factor_used,
    "disabled consistency should not report a direct vertical factor");
  ExpectTrue(!trajectory[1].gnss_factor_used, "disabled sample should not mark trajectory GNSS usage");
}
...
int main() {
  try {
    ...
    RunTest(
      ...
      TestBuilderCanDisableVerticalFactorsWhileKeepingHorizontal);
    RunTest(
      "TestBuilderCanDisableAllGnssFactorsForStage3ReferencePass",
      TestBuilderCanDisableAllGnssFactorsForStage3ReferencePass);
    RunTest(

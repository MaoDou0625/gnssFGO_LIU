...
state.cutoffHz = options.InitialCutoffHz;
...
cutoffSlider = uicontrol(fig, 'Style', 'slider', ...
  ...
  'Min', log10(options.MinCutoffHz), ...
  'Max', log10(options.MaxCutoffHz), ...
...
presetPopup = uicontrol(fig, 'Style', 'popupmenu', ...
  ...
  'String', {'0.005','0.01','0.02','0.03','0.05','0.08','0.10','0.20'}, ...
  'Value', 2, ...
  'Callback', @onPreset);
...
uicontrol(fig, 'Style', 'pushbutton', 'String', 'Save PNG', ...
  'Units', 'normalized', 'Position', [0.06 0.905 0.07 0.032], ...
  'Callback', @onSavePng);
uicontrol(fig, 'Style', 'pushbutton', 'String', 'Save FIG', ...
  'Units', 'normalized', 'Position', [0.14 0.905 0.07 0.032], ...
  'Callback', @onSaveFig);
uicontrol(fig, 'Style', 'pushbutton', 'String', 'Export CSV', ...
  'Units', 'normalized', 'Position', [0.22 0.905 0.08 0.032], ...
  'Callback', @onExportCsv);
statusText = uicontrol(fig, 'Style', 'text', ...
  'Units', 'normalized', 'Position', [0.32 0.902 0.62 0.04], ...
  'String', '', ...
  'BackgroundColor', 'w', ...
...
if nargout == 0
  ...
  function onCutoffEdit(~, ~)
    ...
    state.cutoffHz = min(max(value, options.MinCutoffHz), options.MaxCutoffHz);
    set(cutoffSlider, 'Value', log10(state.cutoffHz));
    set(cutoffEdit, 'String', formatCutoff(state.cutoffHz));
    current = recomputeAndPlot();
  end
  ...
  function onPreset(~, ~)
    ...
    state.cutoffHz = str2double(labels{get(presetPopup, 'Value')});
    set(cutoffSlider, 'Value', log10(state.cutoffHz));
    set(cutoffEdit, 'String', formatCutoff(state.cutoffHz));
    current = recomputeAndPlot();
  end

...
uicontrol(fig, 'Style', 'text', 'String', 'Baseline preset', ...
  'Units', 'normalized', 'Position', [0.75 0.870 0.08 0.025], ...
  'BackgroundColor', 'w', 'HorizontalAlignment', 'left');
baselinePresetPopup = uicontrol(fig, 'Style', 'popupmenu', ...
  'Units', 'normalized', 'Position', [0.835 0.864 0.14 0.032], ...
...
if nargout == 0
  ...
  function onComputeIri(~, ~)
    set(statusText, 'String', 'Computing current smoothed reference 50 m IRI...');
    drawnow;
    try
      iriResult = stage2_iri50_calculator(data, current.referenceUpM, ...
        'ResultDir', resultDir, ...
        'MethodName', char(current.method), ...
        'FirstIndex', current.firstFilterIndex, ...
        'LastIndex', current.lastFilterIndex);
      showIriResultTable(iriResult);
      set(statusText, 'String', sprintf(['%s | IRI mean=%.4g mm/m, max=%.4g mm/m | ' ...
        'csv=%s'], summaryString(data, current, state), ...
        iriResult.meanIriMmPerM, iriResult.maxIriMmPerM, iriResult.csvPath));
    catch ME
      set(statusText, 'String', sprintf('IRI computation failed: %s', ME.message));
      errordlg(ME.message, 'IRI computation failed');
    end
  end
...
data.speedMps = trajectorySpeed(T);
...
if ~isfield(data, 'speedMps') || numel(data.speedMps) ~= numel(data.timeS) || ~any(isfinite(data.speedMps))
  data.speedMps = readTrajectorySpeedForTimes(resultDir, data.timeS);
end
...
function speedMps = trajectorySpeed(T)
speedMps = nan(height(T), 1);
if all(ismember({'vx_mps','vy_mps'}, T.Properties.VariableNames))
  speedMps = hypot(T.vx_mps, T.vy_mps);
  return;
end
if ~all(ismember({'time_s','east_m','north_m'}, T.Properties.VariableNames))
  return;
end
stationM = trajectoryStation(T);
dtS = [nan; diff(T.time_s)];
dsM = [nan; diff(stationM)];
speedMps = dsM ./ dtS;
speedMps(~isfinite(speedMps) | speedMps < 0) = nan;
end
...
function speedMps = readTrajectorySpeedForTimes(resultDir, targetTimeS)
speedMps = nan(size(targetTimeS));
trajectoryPath = fullfile(resultDir, 'trajectory.csv');
if ~isfile(trajectoryPath)
  return;
end
...
sourceSpeedMps = trajectorySpeed(T);
valid = isfinite(T.time_s) & isfinite(sourceSpeedMps);
if sum(valid) < 2
  return;
end
[sourceTimeS, uniqueIndex] = unique(T.time_s(valid), 'stable');
sourceSpeedMps = sourceSpeedMps(valid);
sourceSpeedMps = sourceSpeedMps(uniqueIndex);
speedMps = interp1(sourceTimeS, sourceSpeedMps, targetTimeS, 'linear', nan);
end
...
function showIriResultTable(iriResult)
fig = figure( ...
  'Name', 'Current Smoothed Reference 50 m IRI', ...
  'NumberTitle', 'off', ...
  'Color', 'w', ...
  'Position', [160 120 820 420]);
summaryText = sprintf(['segments=%d | mean=%.4f mm/m | min=%.4f mm/m | ' ...
  'max=%.4f mm/m | distance=%.2f m | tail=%.2f m'], ...
  height(iriResult.table), ...
  iriResult.meanIriMmPerM, ...
  iriResult.minIriMmPerM, ...
  iriResult.maxIriMmPerM, ...
  iriResult.trim.endRawStationM - iriResult.trim.startRawStationM, ...
  iriResult.trim.endRawStationM - iriResult.trim.startRawStationM - ...
    iriResult.table.end_m(end));
uicontrol(fig, 'Style', 'text', ...
  ...
  'HorizontalAlignment', 'left');
uitable(fig, ...
  'Units', 'normalized', 'Position', [0.04 0.22 0.92 0.64], ...
  'Data', table2cell(iriResult.table), ...
  'ColumnName', iriResult.table.Properties.VariableNames);
uicontrol(fig, 'Style', 'text', ...
  'Units', 'normalized', 'Position', [0.04 0.06 0.92 0.10], ...
  'String', sprintf('csv=%s', iriResult.csvPath), ...
  'BackgroundColor', 'w', ...
  'HorizontalAlignment', 'left');
end

...
output = table( ...
    ...
    nav_specific_force_z, nav_acc_z, ...
    'VariableNames', { ...
        ...
        'nav_specific_force_z_mps2', 'nav_acc_z_mps2'});
...
nexttile;
plot(time_rel_s(zoom_mask), nav_specific_force_z(zoom_mask), 'b-', 'LineWidth', 0.9); hold on;
plot(time_rel_s(zoom_mask), nav_acc_z(zoom_mask), 'r-', 'LineWidth', 0.9);
yline(gravity_mps2, 'k--', 'LineWidth', 1.0);
grid on;
xlabel('Relative time (s)');
ylabel('m/s^2');
legend({'nav specific force z', 'nav acceleration z', 'gravity'}, 'Location', 'best');
title('First 20 s Zoom');

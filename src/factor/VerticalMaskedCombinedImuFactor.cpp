...
constexpr std::array<int, VerticalMaskedCombinedImuFactor::kMaskedErrorDim> kKeptErrorRows{
  0, 1, 2, 3, 4, 6, 7, 9, 10, 11, 12, 13, 14};

constexpr double kCovarianceDiagonalFloor = 1.0e-12;

gtsam::Matrix MaskCovariance(const gtsam::Matrix &full_covariance) {
  gtsam::Matrix masked_covariance(
    VerticalMaskedCombinedImuFactor::kMaskedErrorDim,
    VerticalMaskedCombinedImuFactor::kMaskedErrorDim);
  for (std::size_t row = 0; row < kKeptErrorRows.size(); ++row) {
    for (std::size_t col = 0; col < kKeptErrorRows.size(); ++col) {
      masked_covariance(row, col) = full_covariance(kKeptErrorRows[row], kKeptErrorRows[col]);
    }
    if (masked_covariance(row, row) < kCovarianceDiagonalFloor) {
      masked_covariance(row, row) = kCovarianceDiagonalFloor;
    }
  }
...
VerticalMaskedCombinedImuFactor::VerticalMaskedCombinedImuFactor(
  ...
  const gtsam::PreintegratedCombinedMeasurements &preintegrated_measurements)
    : gtsam::NoiseModelFactor6<
        ...
        gtsam::imuBias::ConstantBias>(
        BuildMaskedNoiseModel(preintegrated_measurements),
        pose_i_key,
        velocity_i_key,
        pose_j_key,
        velocity_j_key,
...
gtsam::SharedNoiseModel VerticalMaskedCombinedImuFactor::BuildMaskedNoiseModel(
  const gtsam::PreintegratedCombinedMeasurements &preintegrated_measurements) {
  return gtsam::noiseModel::Gaussian::Covariance(MaskCovariance(preintegrated_measurements.preintMeasCov()));
}

gtsam::Vector VerticalMaskedCombinedImuFactor::MaskErrorVector(
  ...
  gtsam::Vector masked_error(kMaskedErrorDim);
  for (std::size_t row = 0; row < kKeptErrorRows.size(); ++row) {
    masked_error(static_cast<Eigen::Index>(row)) = full_error(kKeptErrorRows[row]);
  }
  return masked_error;
}

void VerticalMaskedCombinedImuFactor::AssignMaskedJacobians(
  ...
  const Eigen::Index cols = bias_jacobian != nullptr
    ? bias_jacobian->cols()
    : preintegration_jacobian.cols();
  output->setZero(kMaskedErrorDim, cols);
...
gtsam::Vector VerticalMaskedCombinedImuFactor::evaluateError(
  ...
  if (h_pose_i) {
    AssignMaskedJacobians(h_pre_pose_i, nullptr, &(*h_pose_i));
  }
  if (h_velocity_i) {
    AssignMaskedJacobians(h_pre_velocity_i, nullptr, &(*h_velocity_i));
  }
  if (h_pose_j) {
    AssignMaskedJacobians(h_pre_pose_j, nullptr, &(*h_pose_j));
  }
  if (h_velocity_j) {
    AssignMaskedJacobians(h_pre_velocity_j, nullptr, &(*h_velocity_j));
  }
  if (h_bias_i) {
    const gtsam::Matrix h_bias_error_i = gtsam::Matrix66::Identity();
    AssignMaskedJacobians(h_pre_bias_i, &h_bias_error_i, &(*h_bias_i));
  }
  if (h_bias_j) {
    const gtsam::Matrix h_pre_bias_j = gtsam::Matrix::Zero(9, 6);
    const gtsam::Matrix h_bias_error_j = -gtsam::Matrix66::Identity();
    AssignMaskedJacobians(h_pre_bias_j, &h_bias_error_j, &(*h_bias_j));
  }

  return MaskErrorVector(preintegration_error, bias_error);
}

...
void ResultWriter::WriteOutputs(
  ...
  if (!result.stage2_mount_leakage_diagnostics.empty()) {
    WriteStage2MountLeakageDiagnosticsCsv(
      output_path / "stage2_mount_leakage_diagnostics.csv",
      result.stage2_mount_leakage_diagnostics);
  }
  if (!result.stage2_vehicle_nhc_state_diagnostics.empty()) {
    WriteStage2VehicleNHCStateDiagnosticsCsv(
  ...
  if (!result.stage3_stage2_increment_hold_diagnostics.empty()) {
    WriteStage3Stage2IncrementHoldDiagnosticsCsv(
  ...
  if (!result.stage3_jump_context_envelope_profiles.empty()) {
    WriteStage3JumpContextEnvelopeProfilesCsv(
      ...
      result.stage3_jump_context_envelope_profiles);
  }
